...
public enum CzkawkaJSON {
    ...
    private static func forEachDuplicate(
        ...
        case .hash:
            ...
            for sizeKey in orderedKeys(bySize.keys, kind: .size) {
                guard let value = bySize[sizeKey] else { continue }
        ...
        case .size, .name:
            ...
            for key in orderedKeys(map.keys, kind: method == .size ? .size : .name) {
    ...
    /// Deterministic order for map-keyed results. czkawka keys its JSON by size (dup HASH, dup
    /// SIZE) or by file name (dup NAME), and a Swift dictionary's iteration order is not part of
    /// its contract, so the same scan could list groups in a different order every run. Sizes sort
    /// biggest first to match how the rest of the app presents reclaimable space; names sort
    /// case-insensitively so the list is stable.
    private static func orderedKeys(
        ...
        case .size:
            return keys.sorted {
                let left = Int($0) ?? -1
                let right = Int($1) ?? -1
                // Non-numeric keys cannot be compared as sizes; fall back to the string form so the
    ...
    private enum OrderedKeyKind { case size, name }
