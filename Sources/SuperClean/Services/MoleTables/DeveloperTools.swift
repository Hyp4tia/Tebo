...
public enum DeveloperTools {
    ...
    public static let all: [CleanTarget] = [
        ...
        adminTarget(
            ...
            kind: .glob,
            explanation: "Stale DeveloperDocumentation index builds. Keeps the newest index; Mole removes the older ones with sudo. SuperClean only reports this row.",
            processGuard: xcodeGuard,
        ...
        adminTarget(
            ...
            "/Library/Developer/CoreSimulator/Caches",
            kind: .directorySweep,
            explanation: "Root-owned CoreSimulator cache entries. Rebuilt when simulators run; Mole needs sudo, SuperClean only reports it.",
            processGuard: simulatorGuard,
        ...
        CleanTarget(
            ...
            kind: .directorySweep,
            risk: .review,
            explanation: "pnpm's global content-addressable store. Mole only prunes it through the pnpm binary (`store prune`, never a raw delete); with no usable pnpm installed it is left for manual review, so SuperClean reports it instead of deleting.",
            reportOnly: true,
