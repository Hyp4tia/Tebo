...
// are deleted through tmutil with a 48-hour safety window, which SuperClean
...
public enum TimeMachine {
    ...
    public static let all: [CleanTarget] = [
        ...
        CleanTarget(
            ...
            risk: .review,
            needsAdmin: true,
            explanation: "*.inProgress/*.inprogress dirs on backup volumes (Backups.backupdb and mounted .backupbundle/.sparsebundle), older than the 48-hour safety window (MOLE_TM_BACKUP_SAFE_HOURS). Upstream deletes them via tmutil; SuperClean only reports them, since they are backup data on external volumes.",
            reportOnly: true,
