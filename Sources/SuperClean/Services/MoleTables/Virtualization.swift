...
public enum Virtualization {
    ...
    public static let all: [CleanTarget] = [
        ...
        CleanTarget(
            ...
            kind: .glob,
            risk: .review,
            explanation: "OrbStack VM/container data. Upstream only reports its size and points at `docker system df` for pruning — the data is daemon-managed, so SuperClean reports it too and never deletes it.",
            reportOnly: true,
