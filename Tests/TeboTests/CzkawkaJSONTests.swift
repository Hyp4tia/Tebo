...
struct CzkawkaJSONTests {
    ...
    /// Tests/TeboTests/.. -> Tests/Fixtures/czkawka
    ...
    @Test("Group order is deterministic across parses, biggest first")
    func groupOrderIsStable() throws {
        // czkawka keys its JSON by size or name, and dictionary iteration order is not a contract:
        // two decodes of the same bytes could list groups differently, which reshuffled the list
        // between runs and made the two entry points disagree.
        let bytes = try data("dup.json")
        let first = try CzkawkaJSON.items(in: bytes, tool: .duplicates).map(\.path)
        let second = try CzkawkaJSON.items(in: bytes, tool: .duplicates).map(\.path)
        #expect(first == second, "the same document must parse in the same order every time")

        let sizes = try CzkawkaJSON.items(in: bytes, tool: .duplicates).map(\.sizeBytes)
        #expect(sizes == sizes.sorted(by: >), "duplicate groups are presented biggest first")
    }
