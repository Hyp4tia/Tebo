...
struct OptimizeTests {
    ...
    func availabilityProbe() {
        ...
        let missing = makeTask(executable: "/nonexistent/tebo-probe-binary")
    ...
    func runnerUnavailable() async {
        let task = makeTask(executable: "/nonexistent/tebo-probe-binary")
    ...
    func runnerSucceeds() async {
        let task = makeTask(
            id: "echo", executable: "/bin/echo", arguments: ["tebo-optimize-marker"]
        )
        let result = await OptimizeRunner().run(task, recordLine: { _ in })
        ...
        #expect(output.contains("tebo-optimize-marker"))
    }
    ...
    func runnerFails() async {
        let task = makeTask(
            ...
            arguments: ["/definitely-missing-tebo-xyz"]
        )
        let result = await OptimizeRunner().run(task, recordLine: { _ in })
