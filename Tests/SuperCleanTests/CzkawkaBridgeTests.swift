...
struct CzkawkaBridgeTests {
    ...
    private static let repoRoot = URL(fileURLWithPath: #filePath)
        .deletingLastPathComponent()  // SuperCleanTests
    ...
    private func tempScanDirs() -> [URL] {
        ...
        ))?.filter { $0.lastPathComponent.hasPrefix("superclean-scan-") } ?? []
    }
    ...
    private func makeFakeEngine(
        ...
        let dir = try makeTempDir("superclean-fake")
    ...
    func missingEngineThrows() async throws {
        let nowhereDir = try makeTempDir("superclean-absent")
        ...
        let tree = try makeTempDir("superclean-tree")
    ...
    func happyPathYieldsItemsAndCleansTemp() async throws {
        let tree = try makeTempDir("superclean-tree")
        let payloadDir = try makeTempDir("superclean-payload")
    ...
    func argumentsAreSafeAndComplete() async throws {
        let tree = try makeTempDir("superclean-tree")
        let payloadDir = try makeTempDir("superclean-payload")
        let argsFile = try makeTempDir("superclean-args").appendingPathComponent("args.txt")
    ...
    func toolNamesPassedVerbatim() async throws {
        ...
        for tool in CzkawkaTool.allCases {
            let argsDir = try makeTempDir("superclean-args")
            ...
            // Fix flags rewrite files in place; SuperClean must never pass them.
    ...
    func cancellationKillsEngineAndCleansTemp() async throws {
        let tree = try makeTempDir("superclean-tree")
        let pidFile = try makeTempDir("superclean-pid").appendingPathComponent("engine.pid")
    ...
    func realEngineEndToEnd() async throws {
        let tree = try makeTempDir("superclean-real")
        defer { try? FileManager.default.removeItem(at: tree) }
    ...
    func realEngineCancellationLeavesNoOrphan() async throws {
        ...
        let tree = try makeTempDir("superclean-real-cancel")
        defer { try? FileManager.default.removeItem(at: tree) }
